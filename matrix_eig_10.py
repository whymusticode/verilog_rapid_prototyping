...
with open('matrix_eig_10__acm_i.txt', 'w') as f:
    write_line(f, acm_i)
with open('matrix_eig_10__acm_q.txt', 'w') as f:
    write_line(f, acm_q)
with open('matrix_eig_10__max_iter.txt', 'w') as f:
...
# Run VHDL (expect matrix_eig_10_fxp.vhd + global_types_pkg.vhd + run_tb.vhd in this dir)
if os.path.isfile('matrix_eig_10_fxp.vhd') and os.path.isfile('global_types_pkg.vhd') and os.path.isfile('run_tb.vhd'):
    d = os.path.dirname(os.path.abspath(__file__)) or '.'
    subprocess.run(['ghdl', '-a', '--std=08', 'global_types_pkg.vhd', 'matrix_eig_10_fxp.vhd', 'run_tb.vhd'], check=True)
    ...
    print('Input/output saved. Copy matrix_eig_10_fxp.vhd and global_types_pkg.vhd here, then re-run to compare with VHDL.')
